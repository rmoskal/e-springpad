...
class OAuthClient():
  ...
  def make_async_request2(self, url, token="", secret="", additional_params=None,
    ...
    payload = self.prepare_request(url, token, secret, additional_params=None,
                                   method=urlfetch.GET)
...
class SpringpadClient(OAuthClient):
  """Dropbox Client.
  ...
  def __init__(self, consumer_key, consumer_secret, callback_url):
    ...
    OAuthClient.__init__(self,
        ...
        consumer_key,
        consumer_secret,
        "http://springpad.com/api/oauth-request-token",
        "http://springpad.com/api/oauth-access-token",
        callback_url)
  ...
  def get_authorization_url(self):
    ...
    return ("http://springpad.com/api/oauth-authorize?"
            "oauth_token=%s&oauth_callback=%s" % (token,
                                                  urlquote(self.callback_url)))

  def _lookup_user_info(self, access_token, access_secret):
    response = self.make_request("http://springpad.com/api/users/me",
                                 token=access_token, secret=access_secret,
                                 protected=True)

    data = json.loads(response.content)
    json_uuid = data['uuid']
    return {'username':data['username'], 'uuid':json_uuid[6:len(json_uuid) - 2]}

  def execute_rpc(self, access_token, access_secret, commands):

    response = self.make_async_request2('http://springpad.com/api/users/me/commands',
                                 token=access_token, secret=access_secret,method=urlfetch.POST, additional_params=json.dumps(commands)).get_result()
    return response
